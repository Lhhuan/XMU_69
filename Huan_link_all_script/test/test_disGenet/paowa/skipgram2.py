...
class Stretching_Potential_Loss(nn.Module):
    ...
    def forward(self,in_embed,out_embed):
        dist = th.norm((in_embed-out_embed).view([-1,in_embed.shape[-1]]),p=2,dim=1)
...
class EmbeddingModel(nn.Module):
    def __init__(self, nodes_nums, embed_size):
        ...
        self.SPL = Stretching_Potential_Loss(1) #拉伸潜在损失
        self.EPL = Electrostatic_Potential_Loss() #静电势损失 


    def forward(self, input_labels, pos_labels, neg_labels,graph_dist):
        '''
        ...
        spl = self.SPL(input_embedding,pos_embedding).sum()
        epl = self.EPL(graph_dist,input_embedding,neg_embedding).sum()
        return spl,epl
