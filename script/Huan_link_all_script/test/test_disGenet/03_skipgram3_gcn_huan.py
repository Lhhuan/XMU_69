...
class GCN(nn.Module):
    ...
    def forward(self, graph, feat):
        with graph.local_scope():
            ...
            # (n, d_in, d_out)
            graph.update_all(fn.copy_u('h', 'm'), self.reducer('m', 'neigh')) #在图的一部分上进行消息传递
...
#-----------------------------------
    def train(self):
        ...
        train_graph = load_graphs(os.path.join(root_path,r'train_graph.dgl'))[0][0].to(cpu)
        ...
        for epoch in range(1,epoches+1):
            ...
            if epoch == 1:
                pd.DataFrame(columns=['epoch','train_epoch_loss','test_epoch_loss']).to_csv(
                    os.path.join(root_path,r'record', rf'03_skipgram03_gcn_date{date}_merics.txt'), sep='\t', index=None)
            pd.DataFrame([epoch,train_epoch_loss,test_epoch_loss]).T.to_csv(
                os.path.join(root_path,r'record', rf'03_skipgram03_gcn_date{date}_merics.txt'), sep='\t', index=None,mode='a',header=None)
            if best_loss > test_epoch_loss:
                print(f"Now best model is epoch{epoch}")
                best_loss = test_epoch_loss
                lookup_table = model.lookup_table()
                np.save(os.path.join(root_path,r'model', rf'03_skipgram03_gcn_date{date}_lookup_table.npy'),lookup_table)
                state = {'net': model.state_dict(), 'optimizer': optimizer.state_dict(), 'epoch': epoch}
                th.save(state,os.path.join(root_path,r'model', rf'03_skipgram03_gcn_date{date}_model.pkl'))
