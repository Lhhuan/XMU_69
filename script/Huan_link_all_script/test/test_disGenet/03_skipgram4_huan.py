...
# from rst_pheno.homograph_word2vec 
import random_walk
    ...
    def train(self):
        ...
        for epoch in range(1,epoches+1):
            ...
            for batched_center_word,batched_pos_word,batched_neg_word in train_bar:
                ...
                batched_pos_word = batched_pos_word[:,3:5].long().to(cpu)
            ...
            if epoch == 1:
                pd.DataFrame(columns=['epoch','train_epoch_loss','test_epoch_loss']).to_csv(
                    os.path.join(root_path,r'skipgram\record', rf'date{date}_merics.txt'), sep='\t', index=None)
            pd.DataFrame([epoch,train_epoch_loss,test_epoch_loss]).T.to_csv(
                os.path.join(root_path,r'skipgram\record', rf'date{date}_merics.txt'), sep='\t', index=None,mode='a',header=None)
            if best_loss > test_epoch_loss:
                print(f"Now best model is epoch{epoch}")
                best_loss = test_epoch_loss
                lookup_table = model.lookup_table()
                np.save(os.path.join(root_path,r'skipgram\model', rf'date{date}_lookup_table.npy'),lookup_table)
#----------------------------------------------
