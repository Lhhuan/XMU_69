...
graph = load_graphs("/public/home/huanhuan/project/GTEx/output/01_undirectied_Graph.dgl")[0][0]
nodes_num = graph.nodes().shape[0]
root_path= '/public/home/huanhuan/project/GTEx/output/'
batch_size = 1000
# nodes_num = 51836
epoches = 100
...
def train():
    ...
    model = EmbeddingModel(nodes_num,256).to(cpu)
    ...
    for epoch in range(1,epoches+1):
        ...
        if epoch == 1:
            pd.DataFrame(columns=['epoch','train_epoch_loss','test_epoch_loss']).to_csv(
                os.path.join(root_path,r'record', rf'03_skipgram_date{date}_merics.txt'), sep='\t', index=None) #-------------r表示raw string，不识别转义, f表示format，用来格式化字符串
            print("11111111111111")
        pd.DataFrame([epoch,train_epoch_loss,test_epoch_loss]).T.to_csv(
            os.path.join(root_path,r'record', rf'03_skipgram_date{date}_merics.txt'), sep='\t', index=None,mode='a',header=None)
        if best_loss > test_epoch_loss:
            print(f"Now best model is epoch{epoch}") 
            best_loss = test_epoch_loss
            lookup_table = model.lookup_table() #------------------------------------------------------------------------取weight .in_embed.weight.
            np.save(os.path.join(root_path,r'model', rf'03_skipgram_date{date}_lookup_table.npy'),lookup_table)
            best_test_loss = test_epoch_loss
            state = {'net': model.state_dict(), 'optimizer': optimizer.state_dict(), 'epoch': epoch}
            # state = model.state_dict()
            th.save(state,os.path.join(root_path,r'model',rf'03_skipgram_date{date}_model.pkl'))

#---------------------------------

# checkpoint_path ="/public/home/huanhuan/project/GTEx/output/model/skip_gram_date08_31_13_08_model.pkl" #model
    ...
    def test(checkpoint_path):#--------------
        ...
        test_size,test_dataloader = load_data('01_test')
...
lookup_table =  np.load("/public/home/huanhuan/project/GTEx/output/model/03_skipgram_date09_10_22_37_lookup_table.npy")
...
def compute_acc(self,lookup_table,source_file):
    ...
    undirected_graph = load_graphs("/public/home/huanhuan/project/GTEx/output/01_undirectied_Graph.dgl")[0][0]
    ...
    test_graph = load_graphs(r'/public/home/huanhuan/project/GTEx/output/01_test_graph.dgl')[0][0]
    test_adjmtx = test_graph.adjacency_matrix().to_dense().numpy()
    test_truth_table = test_adjmtx[:split_point,split_point:]
    del(test_adjmtx)

    neg_links = undirected_truth_table[undirected_truth_table == 0] #为什么不用test_truth_table？
    ...
    d=fpr,tpr,thresholds
    np.save(os.path.join(root_path,r'record', rf'03_skipgram_auc.npy'),d)
#----------------------huan add
roc_auc = auc(fpr,tpr)
plt.figure()
lw=2
...
plt.savefig("/public/home/huanhuan/project/GTEx/output/figure/03_skipgram_auc.png",dpi=600)#保存图片，dpi设置分辨率
plt.show()
#------------------------------
