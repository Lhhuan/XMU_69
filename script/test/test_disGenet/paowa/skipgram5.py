...
class MLP(nn.Module): #Multi Layer Perceptron，即MLP，多层感知器
    def __init__(self, nodes_nums, embed_size):
        super(MLP,self).__init__()
        self.embed = nn.Embedding(nodes_nums,embed_size)
        self.linear1 = nn.Linear(embed_size*2, 256)
        self.bn1 = nn.BatchNorm1d(256)
        self.linear2 = nn.Linear(256,64)
        self.bn2 = nn.BatchNorm1d(64) #BatchNorm就是在深度神经网络训练过程中使得每一层神经网络的输入保持相同分布的
        self.linear3 = nn.Linear(64,1)
    def forward(self,in_idx, out_idx):
        center_embedding = self.embed(in_idx)
        neighbor_embedding = self.embed(out_idx)
        layer = th.cat([center_embedding,neighbor_embedding],axis=-1)
        layer = th.sigmoid(self.linear1(layer))
        layer = self.bn1(layer)
        layer = th.sigmoid(self.linear2(layer))
...
class EmbeddingModel(nn.Module):
    ...
    def forward(self, batched_center_word, batched_pos_word, batched_neg_word):
...
class Train():
    ...
    def generater_pair_sample_and_label(self,batched_center_word,batched_pos_word,batched_neg_word):
        center_word_nums = batched_center_word.shape[0]
        pos_size = batched_pos_word.shape[1]
        neg_size = batched_neg_word.shape[1]
        center_word_nums_p = batched_center_word.repeat([1,pos_size]).flatten()
        center_word_nums_n = batched_center_word.repeat([1,neg_size]).flatten()
        batched_pos_word = batched_pos_word.flatten()
        batched_neg_word = batched_neg_word.flatten()

        batched_center_word = th.cat([center_word_nums_p,center_word_nums_n])
        batched_neigh_word = th.cat([batched_pos_word,batched_neg_word])

        label = th.cat([th.ones(center_word_nums*pos_size),th.zeros(center_word_nums*neg_size)]).cuda().unsqueeze(1)
        return batched_center_word,batched_neigh_word,label
    ...
    def train(self):
        ...
        train_size,train_dataloader = self.load_dat
        ...
        model = MLP(self.nodes_num,256).to(gpu)
        ...
        for epoch in range(1,self.epoches+1):
            ...
            for batched_center_word,batched_pos_word,batched_neg_word in train_bar:
                ...
                batched_neg_word = batched_neg_word.long().to(gpu)
                batched_center_word,batched_neigh_word,batched_labels = self.generater_pair_sample_and_label(batched_center_word,batched_pos_word,batched_neg_word)
                prediction = model(batched_center_word,batched_neigh_word)
                ...
                train_epoch_loss+=loss_cp
                cur_samples += self.batch_size
                train_bar.set_description(f"training epoch {epoch} train_loss {train_epoch_loss/cur_samples:.4f} ")
            train_epoch_loss /= cur_samples
            ...
            for batched_center_word,batched_pos_word,batched_neg_word in val_bar:
                ...
                loss = criterion(prediction,batched_labels)
                loss_cp = loss.item()
                val_epoch_loss+=loss_cp
                cur_samples += self.batch_size
                val_bar.set_description(f"validating epoch {epoch} val_loss {val_epoch_loss/cur_samples:.4f}")
            val_epoch_loss /= val_size
            ...
            pd.DataFrame([epoch,train_epoch_loss,val_epoch_loss]).T.to_csv(
