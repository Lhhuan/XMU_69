...
#-----------------------------------
    def train(self):
        ...
        model = EmbeddingModel(nodes_num,256,8,1).to(cpu) # 实例化一个对象

        train_graph = load_graphs(os.path.join(root_path,r'01_train_graph.dgl'))[0][0].to(cpu)
        # val_graph = load_graphs(os.path.join(root_path,'data',r'val_graph.dgl'))[0][0].to(cpu)
        ...
        for epoch in range(1,epoches+1):
            ...
            for batched_center_word,batched_pos_word,batched_neg_word in train_bar:
                ...
                batched_pos_word = batched_pos_word.long()[:,3:5].to(cpu)
            ...
            if epoch == 1:
                pd.DataFrame(columns=['epoch','train_epoch_loss','test_epoch_loss']).to_csv(
                    os.path.join(root_path,r'record', rf'batch_500_03_skipgram_gcn_date{date}_merics.txt'), sep='\t', index=None)
            pd.DataFrame([epoch,train_epoch_loss,test_epoch_loss]).T.to_csv(
                os.path.join(root_path,r'record', rf'batch_500_03_skipgram_gcn_date{date}_merics.txt'), sep='\t', index=None,mode='a',header=None)
            if best_loss > test_epoch_loss:
                print(f"Now best model is epoch{epoch}")
                best_loss = test_epoch_loss
                lookup_table = model.lookup_table()
                np.save(os.path.join(root_path,r'model', rf'batch_500_03_skipgram_gcn_date{date}_lookup_table.npy'),lookup_table)
                state = {'net': model.state_dict(), 'optimizer': optimizer.state_dict(), 'epoch': epoch}
                th.save(state,os.path.join(root_path,r'model', rf'batch_500_03_skipgram_gcn_date{date}_model.pkl'))
...
def compute_acc(lookup_table,source_file):
    ...
    np.save(os.path.join(root_path,r'record', rf'03_skipgram_gcn_auc.npy'),d)
...
plt.savefig("/public/home/huanhuan/project/GTEx/output/figure/03_gcn_hotspot_egene_auc.png",dpi=600)#保存图片，dpi设置分辨率
plt.show()
